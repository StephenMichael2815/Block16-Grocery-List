...
function calculateTotalInventory() {
  ...
  return items.reduce(
    (total, currentItem) => (total += currentItem.inventory),
    0
  );
}
...
function calculateAllInventoryPrice() {
  ...
  return items.reduce(
    (total, currentItem) =>
      (total += currentItem.inventory * currentItem.price),
    0
  );
}
...
function calculateTotalCartPrice() {
  ...
  return cart.reduce(
    (total, cartId) => (total += items.find((item) => item.id == cartId).price),
    0
  );
}
